...
3. **Guardas antes de retiros masivos**
   - **Modo estabilidad (recomendado para etapa inicial):** con **`IMPORT_WITHDRAW_DISABLE=true`** (default operativo actual), no se ejecutan retiros por feed en cron/webhook/manual. Objetivo: priorizar continuidad de catálogo mientras se estabiliza ingest y calidad de origen.
   ...
   - **Cap porcentual de retiros:** aun con retiros habilitados, si una corrida intenta retirar más de `IMPORT_WITHDRAW_MAX_PERCENT_OF_CATALOG` (default 0,05 = 5%) del catálogo importado, se omiten retiros en esa corrida para evitar borrado masivo accidental.
