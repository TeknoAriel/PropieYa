...
| **Precisión / relevancia** | Resultados percibidos como “justos” | ES: `multi_match` + orden por fecha; **Sprint 47:** `tie_breaker` 0.12 en `best_fields`, boosts `description^0.85` / `address.neighborhood^1.6`; script `pnpm --filter @propieya/web run golden:search-smoke` (apéndice A) |
...
| **Medición** | Saber si mejoramos | `search_history` (usuarios logueados); **`portal_stats_events`:** búsqueda listado, asistente (mensaje + búsqueda OK), comparador, lead, demanda, alerta (`docs/49`); logs `LOG_SEARCH_MS`; golden queries (apéndice); futuro: harness / etiquetado |
...
- **Hechos de producto:** búsqueda listado, comparador, lead desde ficha, perfil de demanda, alertas y asistente (mensaje + búsqueda) en `portal_stats_events` (ver `docs/49`, `PORTAL_STATS_TERMINALS`).  
...
- **Sprint 47:** `multi_match` con `tie_breaker: 0.12` y pesos ligeramente afinados; smoke automatizable: `GOLDEN_SEARCH_BASE_URL` + `pnpm --filter @propieya/web run golden:search-smoke` (`apps/web/scripts/golden-search-smoke.ts`, alineado al apéndice A).
...
**Smoke automatizado (subset):** `GOLDEN_SEARCH_BASE_URL` + `pnpm --filter @propieya/web run golden:search-smoke` — `apps/web/scripts/golden-search-smoke.ts`.
