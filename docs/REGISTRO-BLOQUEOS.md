...
| 2026-03-29 | **Web sin cambios visibles** (`/api/version` queda en `b4067f2`): el código nuevo está en Git pero no en Vercel | **Workflow [Promote deploy/infra](https://github.com/TeknoAriel/PropieYa/actions/workflows/promote-deploy-infra.yml) falló** en **Deploy producción (Vercel…)** en los runs **91** (`8a2e0e7`) y **92** (`a055cbd`); el último **éxito** fue **90** (`b4067f2`). **Un paso:** abrir el run fallido → log del paso **Deploy producción (Vercel build remoto, sin prebuilt)** (token, cuota Hobby, error de build). Corregir y **Re-run failed jobs** o nuevo push a `deploy/infra`. |
